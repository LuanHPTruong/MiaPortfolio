...
function insertText(filepath, id) {
    ...
    fetch(filepath)
        ...
        .then(text => element.innerHTML += parseMd(text));
}


function openInfo(num) {
    const blogText = './blogs/blog' + num + '.md';
    const blogTitle = './blogs/title' + num + '.md';
    insertText(blogTitle, 'blog-title');
    insertText(blogText, 'blog-content');

    document.getElementById('blog-info').style.width = '100%';
}
...
function closeInfo() {
    ...
    // set the size to 0
    document.getElementById('blog-info').style.width = '0';
}
